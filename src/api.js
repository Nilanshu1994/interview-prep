...
// ── Questions — flat, no difficulty, 10 at a time ─────────────────────────────
// Pass existingQuestions so each batch is unique
export async function generateQuestions(topic, existingQuestions = []) {
...
Generate exactly 10 interview questions on the topic "${topic}" for a software engineer with 8 years of experience.
Mix coverage: foundational concepts, intermediate patterns, and advanced/expert scenarios — all in one batch.${existingList}
...
Each item must have exactly these fields:
{
  ...
  "a": "comprehensive answer in plain paragraphs. For code use triple-backtick blocks with language. ASCII diagrams for architecture topics.",
  "tips": ["2-3 specific interview tips for this question — what interviewers look for, common mistakes, likely follow-ups"]
}`;
...
// ── Regenerate one ────────────────────────────────────────────────────────────
export async function regenerateOne(topic, existingQuestions) {
  const existing = existingQuestions.map(q => q.q).join('\n');
  const prompt =
`Generate 1 new interview question on "${topic}" for an 8-year experienced engineer.
...
Return ONLY a raw JSON object — no markdown, no backticks.
{
  ...
  "only_in_a": ["unique feature in ${topicA}"],
  "only_in_b": ["unique feature in ${topicB}"],
  "when_to_use_a": "paragraph on when to choose ${topicA}",
  "when_to_use_b": "paragraph on when to choose ${topicB}"
}
Requirements: 8-10 shared_concepts, real runnable code, 4-6 items in each only_in list.`;
...
Return ONLY a raw JSON object:
{
  ...
  "focus_areas": ["3-5 specific technical areas to prep for"],
  "soft_skills": ["if mentioned"]
}`;
...
Generate 15 targeted questions specifically for THIS role — referencing specific technologies and scenarios from the JD.

Return ONLY a raw JSON array. No markdown. Start with [ end with ]
Each item:
{
  "q": "specific, role-targeted question",
  "a": "detailed answer tailored to this role and tech stack",
  "tips": ["tip specific to this role context","what this company likely cares about"]
}`;
